...
{
    // (DTOs FacturaItemDto, FacturaDto, VentaListado... todo igual)
    #region DTOs
    ...
    {
        // (Ya no necesitamos _prodDatos aquí)
        ...
        public FacturaDto ObtenerFacturaPorId(int idVenta)
        {
            ...
            try
            {
                        ...
                        u.Nombre_Usuario, u.Apellido_Usuario;", cn))
                {
                    ...
                    if (!dr.Read())
                        throw new InvalidOperationException($"No se encontró la factura con ID {idVenta}.");
                    // (Mapeo de factura...)
                    ...
                    if (factura.Envio) { factura.CostoEnvio = COSTO_ENVIO_FIJO; factura.Subtotal = Math.Max(0, factura.Total - COSTO_ENVIO_FIJO); }
                    else { factura.CostoEnvio = 0m; factura.Subtotal = factura.Total; }
                }
            ...
            {
                throw new Exception("Error al obtener los datos de la factura desde la base de datos.", ex);
            }
        }
        ...
        public List<VentaListado> ObtenerListadoPorUsuario(int idUsuario)
        {
            ...
            if (idUsuario > 0)
            {
                sqlQuery += " WHERE v.id_Usuario = @idUsuario ";
            }
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    lista.Add(new VentaListado
                    {
                        ...
                        VendedorNombreCompleto = $"{dr.GetString(7)} {dr.GetString(8)}"
                    });
                }
                return lista;
            }
            ...
            {
                throw new Exception("Error al obtener el listado de ventas.", ex);
            }
        }
        ...
        public void InsertarVentaConItems(Venta venta, List<ItemVenta> items)
        {
            ...
            try
            {
                ...
                // 3) Ítems (El trigger 'trg_Item_Venta_AjustarStock' descuenta el stock)
        ...
        {
            // Gracias a "ON DELETE CASCADE" y al Trigger de Stock,
            // solo necesitamos ejecutar esto.
            try
            {
                using var cn = Conexion.ObtenerConexion();
                using var cmd = new SqlCommand(@"DELETE FROM Venta WHERE id_Venta = @id;", cn);
                ...
                if (rowsAffected == 0)
                {
                    throw new InvalidOperationException("No se encontró la venta para anular. Es posible que ya haya sido eliminada por otro usuario.");
                }
            }
            catch (SqlException ex)
            {
                // Error de base de datos
                throw new Exception("Error de base de datos al intentar anular la venta.", ex);
            }
            catch (Exception)
            {
                // Relanza el InvalidOperationException o cualquier otro
                throw;
            }
        }
        // ---------- FIN MÉTODO NUEVO ----------
